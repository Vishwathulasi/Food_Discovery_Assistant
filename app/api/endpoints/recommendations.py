...
def _is_veg_place(place: dict) -> bool:
    """
    Veg-only detection based ONLY on the dataset's cuisine/category fields.

    Rule:
    - If cuisine or category contains the word 'vegetarian' (case-insensitive)
      → treat as veg restaurant.
    - Otherwise → NOT veg.

    This matches the Kaggle CSV convention:
      e.g. "South Indian, North Indian, Vegetarian"
    """
...
def recommend():
    ...
    attrs["veg_only"] = effective_veg_only  # keep attrs in sync
    # ---------------------------------------------------
    ...
    if not filtered:
        # Special message when user clearly wanted veg-only
